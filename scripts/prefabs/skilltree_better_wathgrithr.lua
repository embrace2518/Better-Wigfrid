...
local function BuildSkillsData(SkillTreeFns)
    local skills =
    {
        ...
        wathgrithr_allegiance_shadow = {
            ...
            onactivate = function(inst)
                ONACTIVATE_FNS.AllegianceShadow(inst)
            end,
            ondeactivate = function(inst)
                ONDEACTIVATE_FNS.AllegianceShadow(inst)
            end,
        },
        ...
        wathgrithr_allegiance_lunar = {
            ...
            onactivate = function(inst)
                ONACTIVATE_FNS.AllegianceLunar(inst)
            end,
            ondeactivate = function(inst)
                ONDEACTIVATE_FNS.AllegianceLunar(inst)
            end,
        },
    }
    ...
    for name, data in pairs(skills) do
        ...
        data.desc = data.desc or STRINGS.SKILLTREE[uppercase_name.."_DESC"]

        -- If it's not a lock.
        if not data.lock_open then
            data.title = data.title or STRINGS.SKILLTREE[uppercase_name.."_TITLE"]
